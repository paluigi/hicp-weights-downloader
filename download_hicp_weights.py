...
# CONFIGURATION
# ===========================
DATAFLOW_ID = "PRC_HICP_IW"   # Eurostat dataflow for HICP Item Weights ECOICOP v2
OUTPUT_DIR = Path("src", "assets", "data")
MAPS_DIR = Path("src", "assets", "maps")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

FREQ = "A"                     # Annual frequency for yearly weights
START_PERIOD = "2000"          # Start year
...
def download_data(item_id, dimensions, FREQ, START_PERIOD, DATAFLOW_ID):
    """Download yearly weight data for a specific ECOICOPv2 item and return a DataFrame."""
    
    # Same countries and areas as the reference script
    geo_key = "EU+EA+BE+BG+CZ+DK+DE+EE+IE+EL+ES+FR+HR+IT+CY+LV+LT+LU+HU+MT+NL+AT+PL+PT+RO+SI+SK+FI+SE"
    
    # Parts per thousand - the standard unit for HICP weights
    unit_key = "PER_THS_TOT" 

    # Detect dimensions dynamically based on the DSD (Data Structure Definition)
...
if __name__ == "__main__":
    ...
    # LOAD DATA MAPS
    # ===========================
    csv_path = MAPS_DIR / "coicop18.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"Missing input CSV: {csv_path}")
        
    code_map = pd.read_csv(csv_path)

    # ===========================
    # GET STRUCTURE
    # ===========================
    print(f"Downloading data structure for {DATAFLOW_ID}...")
    try:
    ...
    # ===========================
    all_data = []
    total_items = len(code_map["code"])
    
    for i, item_id in enumerate(code_map["code"]):
        print(f"\nProcessing item {i+1}/{total_items}: {item_id}")
        try:
    ...
    if all_data:
        ...
        print(f"\nSuccessfully saved weights to {output_file}")
    else:
